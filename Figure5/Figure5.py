...
def take_log(original_value):
    vals = original_value[original_value > 0]
...
metrics = ['Change Modulation Index', 'Time to first spike', 'Baseline Rate', 'Pre-change Response','Change Response']
labels = metrics
bins = [np.linspace(-1,1,30), np.linspace(30,140,30), np.linspace(-2,2,50), np.linspace(0,10,50), np.linspace(0,10,50)]
function_to_apply = [do_not_change, convert_to_ms, take_log, do_not_change, do_not_change]
y_vals = [0.2,60,0.6,4,4]
...
for area_idx, area in enumerate(areas):
    ...
    mice_per_area[area_idx] = len(df[selection]['Mouse ID'].unique())
